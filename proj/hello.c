...
#include <string.h>
char stuName[20][30];
float presentAttendance[20]={0};
float absentAttendance[20]={0};
float indexNumber=0;
float percentage[20]={0};
void checkAttendance()
{
    int i;
    printf("\n               Total Present | Total Absent | Percentage\n");   
    for( i = 0; i < indexNumber; i++)
    {        
        printf("%s",stuName[i]);
        printf("              %.1f             %.1f           %.1f\n",presentAttendance[i],absentAttendance[i],percentage[i]); 
        if(percentage[i]<75){
            printf("\tRemark -------> Attendance short       \n");
        }                          
    }    
}
void addStudent()
...
void takeAttendance()
{
    ...
    int i;
    char ch;
     printf("enter the class number:");  // class number starts from 0
    scanf("%d",&n);
    if(n>-1){
        n+=1;
    }
    ...
    for(i=0; i<indexNumber; i++){
        percentage[i]=(presentAttendance[i]/n)*100;
    }
